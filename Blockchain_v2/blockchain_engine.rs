...
// Version details
const CHRONO_VERSION: &str = "0.4.23";
const SHA256_VERSION: &str = "1.1.1";
const CRYPTO_HASH_VERSION: &str = "0.3.4";
...
impl BlockChain {
    ...
    fn chain_selector(&self, local: Vec<Block>, remote: Vec<Block>) -> Option<Vec<Block>> {
        ...
        match (is_local_valid, is_remote_valid) {
            ...
            (true, false) => { 
                println!("The remote copy is invalid, returning local copy");
                Some(local)
            }, 
            (false, true) => {
                println!("The local copy is invalid, returning remote copy");
                Some(remote)    
            },
            (false, false) => {
